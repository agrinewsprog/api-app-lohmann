...
import { loadProducts } from '../products/productionProducts.loader';
...
export class ProductionFlocksService {
  ...
  async getAllFlocks(userId: number, farmId?: number): Promise<ProductionFlockResponse[]> {
  ...
  async createFlock(userId: number, data: CreateProductionFlockDTO): Promise<ProductionFlockResponse> {
    ...
    if (data.farmId) {
      const farm = await this.farmsRepository.findByIdAndUserId(data.farmId, userId);
      if (!farm) {
        throw new AppError(400, 'Farm not found or does not belong to you');
      }
    }

    // Validate productId exists in products.json if provided
    if (data.productId) {
      const products = loadProducts();
      const productExists = products.some(p => p.id === data.productId);
      if (!productExists) {
  ...
  async updateFlock(id: number, userId: number, data: UpdateProductionFlockDTO): Promise<ProductionFlockResponse> {
    ...
    // Validate productId exists if being changed
