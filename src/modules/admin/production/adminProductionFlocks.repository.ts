...
import { ProductionFlock, AdminCreateProductionFlockDTO, AdminUpdateProductionFlockDTO } from './adminProductionFlocks.types';
...
export class AdminProductionFlocksRepository {
  async findAll(
    ...
    const [rows] = await query<ProductionFlock[] & RowDataPacket[]>(sql, params);
    return rows;
  }
  ...
  async update(id: number, data: AdminUpdateProductionFlockDTO): Promise<ProductionFlock | null> {
    const fields: string[] = [];
